...
- **Приема заявок**: бот принимает обращение и ведет диалог по одному тикету
- **Уточнения проблемы**: если данных мало, бот задает уточняющие вопросы
- **Поиска по базе знаний**: если решение найдено в `Knowledge Base`, бот сразу отвечает пользователю
- **Проверки известных проблем**: если ответа нет, бот ищет похожую запись в `issues.json`
- **Дедупликации и эскалации**: дубль не создается повторно, иначе формируется новая задача
...
- **Staged dedup**: сначала быстрые лексические сравнители, затем LLM только для спорных случаев
- **Проверка quality**: есть тесты и benchmark для сравнения алгоритмов лексических сравнений
- **Чистые слои**: бизнес-логика в `service/`, доступ к данным в `repo/` через абстракции
- **Структурированная классификация**: LLM возвращает категорию, приоритет и вопрос в строгом формате
- **Простой runtime**: файловая KB, `issues.json` и единый `state` без лишней инфраструктуры
